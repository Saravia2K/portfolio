  ...
  return (
    <>
      <h1 className="text-center text-5xl font-bold uppercase sm:text-6xl lg:text-start xl:text-7xl">
        {m.title_part_one()}{" "}
        <span className="text-chart-4">{m.title_part_two()}</span>
      </h1>
      <h2 className="mt-4 text-justify text-[1rem] sm:text-[1.25rem] xl:text-start">
        <span className="text-chart-1">{m.description_quote()}</span>
        <span className="mt-3 block text-[1rem]">
          {m.description_part_one({ yearsOfExperience: yearsOfExperiencie })}
        </span>
        <span className="mt-3 block text-[1rem]">
          {m.description_part_two()}
        </span>
      </h2>
      <div className="mt-5 flex items-center gap-4">
        ...
        <span className="text-[0.9rem] sm:text-2xl">{m.yoe_quote()}</span>
      </div>
      <MainStack />
      <Techs />
      <Education />
